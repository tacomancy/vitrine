...
**The write API** (same module, #121): `write(vaultPath, path, { operations, basedOn })` runs the protocol above for the splicing operations and answers `{ written: true, hash, content }` or `{ written: false, reason, detail }` with `reason` ∈ `changedAndUnreapplyable | verificationFailed | changedOnDisk | noFrontmatter | unreadable | alreadyExists`; an I/O failure at the rename is a `writeFailed` VaultError, as at capture. `setFrontmatter` is `{ keys?, addTags? }`: keys through `yaml`'s Document with `lineWidth: 0` so an untouched long scalar is never folded; every `setFrontmatter` in one write edits one Document, so the frontmatter is one splice. `addTags` skips a tag the file already carries under any casing (by canonical form), so the writer never introduces a variant of a tag the user has. Verification today: the result parses, `kind` is unchanged (a file that had none gets none this way), no owned section is *newly* duplicated — one the file already had is reported on the read, not refused. `replaceFile(vaultPath, path, content, basedOn)` and `createFile(vaultPath, path, content)` are exported beside it; `replaceFile`'s fence (the Vault editor only) is its doc comment. No router procedure writes yet.
