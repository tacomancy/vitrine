...
- **−** The masthead and `<head>` are hand-kept on five pages now, not three. ADR 0004's "no generator until a page needs one" still holds — the copies are identical apart from path depth and `aria-current` — but the count is the number to watch; a sixth page is the moment to price an include step in `build-site.sh`.
