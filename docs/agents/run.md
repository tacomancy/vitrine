...
Copy `packages/core/fixtures/obsidian-vault` into the scratchpad: it has `.obsidian/`, a note, one Question outside `questions/` (so an open shows a row before anything is captured), one freshly promoted Research Question under `questions/` for `#/questions/…`, and under `sources/` a Source whose PDF is there, one whose `pdf:` names a file that is not, and a stub — the three states the picker's rows distinguish. Never point a run at the user's real vault. A failing write is simulated with `chmod 500` on `questions/`; restore it after.
