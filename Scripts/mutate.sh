...
# everything .gitignore does not exclude, plus nothing from .git or
# .claude, whose worktrees/ holds whole sibling checkouts — under build/,
# where the mutated copy then lands too. The test command itself is in
# Scripts/muter.conf.yml.
#
# Usage: Scripts/mutate.sh [report.json]
set -euo pipefail

MUTER_REPOSITORY="https://github.com/muter-mutation-testing/muter.git"
# Head of muter#309, on master at 7f1f258 (muter#306, Swift Testing failure
# detection).
MUTER_COMMIT="5be334dda0caef6fedfb1a4ea46b4072da3a7dc1"

repo_root="$(cd "$(dirname "$0")/.." && pwd)"
build_dir="$repo_root/build"
muter_binary="$build_dir/muter/bin/muter-$MUTER_COMMIT"
...
if [[ ! -x "$muter_binary" ]]; then
    ...
    # The sandbox flag is how Muter's own Makefile builds it.
    swift build --package-path "$source_dir" -c release --product muter --disable-sandbox
    mkdir -p "$(dirname "$muter_binary")"
    cp "$source_dir/.build/release/muter" "$muter_binary"
fi
...
# Muter's coverage step runs the whole suite once more, only to leave files
# no test reaches unmutated; the coverage gate already holds the package at
# its floor (ADR 0006), so the run is skipped and every file is mutated.
(
    cd "$staged_repo"
    "$muter_binary" run \
